...
`}
function Te(t){const e=document.querySelector(".session-banner");if(e)return;const a=document.querySelector(".hero")||document.querySelector(".report-topbar");if(!a)return;a.insertAdjacentHTML("afterbegin",`
...
`);const n=document.querySelector("#signOutButton");n&&n.addEventListener("click",async()=>{try{await signOut()}catch(o){window.alert(`退出登录失败：${o instanceof Error?o.message:String(o)}`)}})}
async function Re(){if(window.location.pathname==="/auth/callback"){Le("正在完成登录...");const t=await handleAuthCallback(),e=((t.state==null?void 0:t.state.returnTo)||"/").toString();window.location.replace(e);return null}const t=await getAuthUser();return t?(Ie(),Te(getUserDisplayName(t)),t):(Le("正在跳转到 Keycloak 登录页..."),await redirectToSignIn(),null)}
async function Oe(){try{Le();await Re()}catch(t){Ae(t instanceof Error?t.message:String(t))}}
Oe();
